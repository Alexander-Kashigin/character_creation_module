...
def defence(char_name: str, char_class: str) -> str:
    """Функция защиты от атак."""
    if char_class == 'warrior':
        blocking: int = 10 + randint(5, 10)
    if char_class == 'mage':
        blocking: int = 10 + randint(-2, 2)
    if char_class == 'healer':
        blocking: int = 10 + randint(2, 5)
    return (f'{char_name} блокировал {blocking} урона')


def special(char_name: str, char_class: str) -> str:
    """Функция спец. возможностей."""
    if char_class == 'warrior':
        print(f'{char_name} применил специальное умение '
              f'«Выносливость {80 + 25}»')
    if char_class == 'mage':
        print(f'{char_name} применил специальное умение «Атака 45»')
    if char_class == 'healer':
        print(f'{char_name} применил специальное умение «Защита 40»')
